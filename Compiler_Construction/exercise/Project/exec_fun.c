...
value_t handle_plus(value_t left, value_t right, value_t result){
	if (left.type == 0 && right.type == 0) { // int + int
        result.type = 0;
        result.u.i = left.u.i + right.u.i;
    } else if (left.type == 1 && right.type == 1) { // double + double
        result.type = 1;
        result.u.d = left.u.d + right.u.d;
    } else if ((left.type == 0 && right.type == 1) || (left.type == 1 && right.type == 0)) { // int + double
    	printf("Error: Cannot add int and double directly!\n");
    	exit(EXIT_FAILURE);
    } else if (left.type == 2 && (right.type == 0 || right.type == 1)) { // string + int/double
        result.type = 2;
        double val = (right.type == 0) ? right.u.i : right.u.d;
    ...
    } else if ((left.type == 0 || left.type == 1) && right.type == 2) { // int/double + string
        result.type = 2;
        double val = (left.type == 0) ? left.u.i : left.u.d;
        int int_part = (int)fabs(val);
        int i = 1;
        while (int_part >= 10) {
            int_part /= 10;
...
void handle_assignment(ast_type *node, Scope *current_scope) {
    ...
    // Aggiorna il valore nella symbol table
    if (var->type == 2) { // Tipo stringa
        free(var->u.s); // Libera la memoria precedente
        var->u.s = strdup(new_value.u.s);
    } else {
        *var = new_value; // Copia il valore per tipi numerici
    }
}
